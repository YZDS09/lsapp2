...
class DashboardModule:
    ...
    def render(self):
        ...
        # 自定义图表制作
        st.divider()
        st.subheader("自定义图表制作")
        self._render_custom_charts(df)
...
#==========================================================================
    # 自定义图表制作
    def _render_custom_charts(self, df):
        """渲染自定义图表制作功能"""
        # 获取数值列（用于Y轴）
        numeric_cols = [col for col in df.columns if pd.api.types.is_numeric_dtype(df[col])]
        
        if not numeric_cols:
            st.warning("数据中没有数值列，无法创建图表")
            return
            
        # 图表类型选择
        chart_type = st.selectbox(
            "选择图表类型",
            ["折线图", "柱状图", "饼图", "散点图", "面积图", "热力图"],
        ...
        if chart_type == "饼图":
            # 饼图特殊配置
            self._render_pie_chart_config(df, numeric_cols)
        elif chart_type == "热力图":
            # 热力图特殊配置
            self._render_heatmap_config(df, numeric_cols)
        else:
            # 通用图表配置
            self._render_general_chart_config(df, numeric_cols, chart_type)
    
    def _render_general_chart_config(self, df, numeric_cols, chart_type):
        """渲染通用图表配置（折线图、柱状图、散点图、面积图）"""
        ...
        # 图表标题
        chart_title = st.text_input("图表标题", f"自定义{chart_type}")
        
        # 生成图表按钮
        if st.button("生成图表", key="generate_chart_btn"):
            if not y_axes:
                st.warning("请至少选择一个Y轴数据")
            else:
                self._draw_custom_chart(df, x_axis, y_axes, chart_type, chart_title)
    
    def _render_pie_chart_config(self, df, numeric_cols):
        """渲染饼图配置"""
        # 让用户选择饼图的数据行
        ...
        # 图表标题
        chart_title = st.text_input("图表标题", f"{selected_row} 饼图")
        
        # 生成图表按钮
        if st.button("生成饼图", key="generate_pie_btn"):
            if not pie_cols:
                st.warning("请至少选择一个数据列")
            else:
                self._draw_pie_chart(df, pie_cols, selected_row, chart_title)
    
    def _render_heatmap_config(self, df, numeric_cols):
        """渲染热力图配置"""
        # 选择热力图的数据列
        ...
        # 图表标题
        chart_title = st.text_input("图表标题", "相关性热力图")
        
        # 生成图表按钮
        if st.button("生成热力图", key="generate_heatmap_btn"):
            if len(heatmap_cols) < 2:
                st.warning("请至少选择两个数据列")
            else:
                self._draw_heatmap(df, heatmap_cols, chart_title)
    
    def _draw_custom_chart(self, df, x_axis, y_axes, chart_type, title):
        """绘制自定义图表"""
        fig = None
        ...
        try:
            if chart_type == "折线图":
                ...
                # 散点图只支持一个Y轴
                if len(y_axes) > 1:
                    st.warning("散点图只支持一个Y轴，已使用第一个选择的Y轴")
                fig = px.scatter(
                    df, 
                    x=x_axis, 
                    y=y_axes[0],
    ...
    def _draw_pie_chart(self, df, cols, selected_row, title):
        """绘制饼图"""
        try:
            # 获取选中的行数据
            row_data = df[df["指标名称"] == selected_row].iloc[0]
            
            # 获取各部分的值
            values = [row_data[col] for col in cols]
            labels = [f"{col} ({val})" for col, val in zip(cols, values)]
...
#================================ 添加图标类型==================================
                        ...
                        # # 饼图
                            ...
                            selected_row = st.selectbox(
                                "选择数据行",
                                options=df["指标名称"],
                                #index=1 if len(row_options) > 1 else df["指标名称"][0],
                                key=f"row_select_{chart_name}"
                            )
                            row_data = df[df["指标名称"]== selected_row]
